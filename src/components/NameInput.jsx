...
import { NAME_MAX_CHARS } from '../utils/nameSafety'
...
function limitChars(value) {
  return Array.from(value).slice(0, NAME_MAX_CHARS).join('')
}
  ...
  return (
    <div className="app-quiz-name-wrap">
      <input
        ...
        onBlur={handleBlur}
        onKeyDown={(event) => {
          if (event.key !== 'Enter' || composingRef.current || disabled) return
      ...
      />
      <p className="app-quiz-name-count">
        {counted}/{NAME_MAX_CHARS}
      </p>
    </div>
  )
}
