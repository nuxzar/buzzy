...
function bezierEase(t, x1, y1, x2, y2) {
  ...
  for (let i = 0; i < 8; i += 1) {
    const ix = 1 - u;
    const x = 3 * ix * ix * u * x1 + 3 * ix * u * u * x2 + u * u * u;
    const dx =
      3 * ix * ix * x1 + 6 * ix * u * (x2 - x1) + 3 * u * u * (1 - x2);
...
function rasterizeImage(blob, size = RASTER_SIZE) {
...
function roundLoopCorners(points, radius) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let s = 0; s <= steps; s++) {
      ...
      const c = t * t;
      out.push(
        new THREE.Vector2(a * p0.x + b * curr.x + c * p1.x, a * p0.y + b * curr.y + c * p1.y)
      );
    }
  }
...
function shapesFromImage(data) {
  ...
  for (let i = 0; i < loops.length; i++) {
    ...
    const shape = new THREE.Shape(loops[i].map(([x, y]) => new THREE.Vector2(x, y)));
    shapes.push(shape);
    owners.push({
      loop: loops[i],
  ...
  for (let i = 0; i < loops.length; i++) {
    ...
    owner?.shape.holes.push(new THREE.Path(loops[i].map(([px, py]) => new THREE.Vector2(px, py))));
  }
  return shapes;
}
...
function textureFromImageData(data) {
  bleedColors(data);
  const canvas = document.createElement("canvas");
...
export function createLiquidObject(elements, options = {}) {
  ...
  function rememberLook(material) {
  ...
  const passMesh = new THREE.Mesh(quad, blankPass);
  ...
  function makeSimTarget(size, format) {
  ...
  function splat(
    ...
    (splatPass.uniforms.uPoint.value).copy(pointer);
    (splatPass.uniforms.uValue.value).copy(value);
  ...
  function applyOptions() {
    ...
    compositePass.uniforms.uAberration.value = Math.min(Math.max(config.aberration, 0), 1);
    compositePass.uniforms.uGrain.value = Math.min(Math.max(config.grain, 0), 1);
    ...
    if (config.background) {
      ...
      (compositePass.uniforms.uBackground.value).set(backgroundColor.r, backgroundColor.g, backgroundColor.b);
  ...
  function resize() {
    ...
    sceneTarget.setSize(Math.max(1, Math.round(width * pr)), Math.max(1, Math.round(height * pr)));
  ...
  function tick(time) {
    ...
    if (!reducedMotion) {
      ...
      const recede = swim > 0 ? bezierEase(swim, ...SWIM_EASE) : 0;
      if (swim > 0) {
        camera.position.setLength(config.cameraDistance + recede * 26);
        canvas.style.opacity = String(Math.max(0, 1 - recede));
      }

      const wagFade = 1 - recede;
      const happyWag = happy ? Math.sin(elapsed * 4.4) * 0.52 * wagFade : 0;
      const happyRoll = happy ? Math.sin(elapsed * 4.4) * 0.1 * wagFade : 0;
      const happyBob = happy
    ...
    (compositePass.uniforms.uCursor.value).copy(cursorUv);
    ...
    if (swim >= 1 && swimOnDone) {
      const done = swimOnDone;
      swimOnDone = null;
  ...
  function react() {
    ...
    for (const id of reactTimers) window.clearTimeout(id);
    reactTimers.length = 0;

    const kicks = [
      { x: 2.45, y: -1.8, squash: -1.1, delay: 0 },
      { x: -2.2, y: 1.55, squash: 0.42, delay: 85 },
      { x: 1.55, y: -1.05, squash: -0.38, delay: 170 },
    ];

    for (const kick of kicks) {
      const id = window.setTimeout(() => {
        if (disposed) return;
        wobbleTiltVel.x += kick.x;
        wobbleTiltVel.y += kick.y;
        squashVel += kick.squash;
        startLoop();
      }, kick.delay);
      reactTimers.push(id);
    }

    const strength = Math.max(config.splash, 0) * 0.5;
    if (strength > 0) {
      for (let i = 0; i < 8 && queued.length < 64; i++) {
        const angle = (i / 8) * Math.PI * 2;
        queued.push([
          0.5 + Math.cos(angle) * 0.018,
          0.52 + Math.sin(angle) * 0.018,
          Math.cos(angle) * 55 * strength,
          Math.sin(angle) * 55 * strength,
          2,
        ]);
      }
    }
...
export function LiquidObject({
  ...
  useImperativeHandle(ref, () => ({
    react() {
      instanceRef.current?.react();
    },
    startHappy() {
      instanceRef.current?.startHappy();
    },
    startSwimAway(onDone) {
      instanceRef.current?.startSwimAway(onDone);
    },
  }));
  ...
  useEffect(() => {
    ...
    if (!instanceRef.current) {
      initialOptions.onError?.(new Error("WebGL unavailable"));
    }
    return () => {
